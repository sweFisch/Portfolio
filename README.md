<!-- Formatting Docs -->
<!-- https://docs.github.com/en/get-started/writing-on-github/getting-started-with-writing-and-formatting-on-github/basic-writing-and-formatting-syntax -->
# Niklas Fischer Portfolio
### About me
I'm a games programmer that previously worked as a 3D generalist for commercials, visualizations and architecture, primarily in the civil engineering space.

Thanks for checking out my game portfolio!

Click the name of the project read more!

#### CV
[If you are interested of what I have done before you can check out my general - CV](CV#cv-site)


# Games

<!-- SOKOBAN start -->
## [***Sokoban***](Sokoban#sokoban)

<img src=".\Images\Sokoban-KeyArt.jpg"/>

```
Game      : Sokoban
Role      : Gameplay Programmer, Sprite artist
When      : 2026 August - 2026 September
Engine    : C++ and SDL
Team size : solo -> growing to a 2 person team

School assignment to create a sokoban game using the simple game engine built
from scratch in C++ primarily using SDL as a library.

Engine features:
Custom memory arena and data oriented entities
Hot reload - game logic DLL - while game is running

I have done extensions to the base engine by: 
Adding an level editor in engine. (<- wip)
Implemented my own rule tile code. (<- wip)
Made changes to the rendering and animation system.
And as a team we have added our own gameplay logic and sprites.

```
> [!NOTE]
> This game and site is WIP
>
> [Read more about the Sokoban project](Sokoban#sokoban)
<!-- > ## [*Read more about Template-Name*](Template-Name#template-Name) -->
<!-- > [Template-Name - Itch Link](https://yrgo.itch.io/Template-Name) -->
---
<!-- end of SOKOBAN  -->

<!-- Mobile Game - start  -->
## [***Mobile Game***](MobileGame#mobile-game)

<img src=".\Images\MobileGame-KeyArt.jpg"/>

```
Game      : Mobile Game
Role      : Gameplay Programmer/ Designer ect
When      : 2026 january - 2026 february
Engine    : Unity
Team size : solo

Solo school assignment to do a mobile game between lectures about programming patterns.
I focused on the player control feel and getting the custom procedural level generator
to be controllable.
Random challenges but with a ordered flow so it wont be impossible to play.
Built using a pooling system and a custom level chunk structure within prefabs.
Data objects primarily holding information about what objects to ask the pooling system for.
Using scriptable objects and different managers to create the level on the fly.
Player controller using primarily physics casts (raycast and capsule casts), 
a player state machine, swipe input handling.
A modular power-up system that ties into the UI and is extendable.
A inverted game where the camera never moves but the world scrolls past it,
using a central game manager controlling much of the game.
Made a curve world shader using shader graph and the game assets using some simple 3D modeling.
Highscore saving to Firebase Database.

```
> [!NOTE]
>
> This site is still wip. \
> [*Read more about the Mobile Game Project*](MobileGame#mobile-game)
<!-- > [Template-Name - Itch Link](https://yrgo.itch.io/Template-Name) -->

---
<!-- Mobile Game End  -->

<!-- Florida man on the Run - START - -->
## [***Florida Man On The Run***](FloridaManOnTheRun#florida-man-on-the-run)

<!-- <img src="Images\FloridaMan-KeyArt.jpg"/> -->
<img src=".\FloridaManOnTheRun\Images\FloridaMan_KeyArt.webp"/>


```
Game      : Florida Man On The Run!
Role      : Gameplay Programmer, Project Manager, Producer
When      : 2025 december - 2026 january
Engine    : Unity
Team Size : 7 - (3 programmers)

Game pitch and idea by me. 
Inspired by very old games like TonyHawk Pro skateboarder and other extreme sport games. 
Made within the restrictions of a small team during 8 weeks.
Primary programming was on the character controller, using a state machine and an input wrapper.
Small art and level tools, camera parallax, tutorial prompts and triggers, 
camera shake, score manager and score UI.
2D animation integration.

```
> [!NOTE]
> Read more about : [Florida Man On The Run](FloridaManOnTheRun#florida-man-on-the-run)
>
> Itch Link : [Check out itch site ](https://yrgo-game-creator.itch.io/florida-man-on-the-run)

---
<!-- Florida man on the run - End -  -->


<!-- Pale Reflections Start -->
## [***Pale Reflections***](PaleReflections#pale-reflections)

<!-- <img src="Images\PaleReflections-KeyArt.jpg"/> -->

<img src=".\PaleReflections\img\pr_keyArt.webp"/>

```
Game      : Pale Reflections
Role      : Gameplay Programmer, AI implementation
When      : 2026 march - 2026 june
Engine    : Unreal Engine
Team Size : 6 people - (3 programmers)

A stealth/horror game where you only see the monsters through the mirrors.
My primary role was building the main enemy AI and the roof enemy logic.
The main monster has sight, hearing, patrolling and different states
affecting speed, animations and behavior.
The AI states was done using blueprints and the different Unreal Engine tools
for pathfinding and behaviors.
I made and implemented the blood camera shader using Unreal node material system.
Roof Cave in particle systems and trigger events.
Particle systems for vial explosions.
And all the integration with the main enemy AI of the above.
The work also involved talking to the creature artist and planning 
for the different animations needed and possible behavior.
We also created the death cutscene together.

```
> [!NOTE] 
> Read more about : [PaleReflections](PaleReflections#pale-reflections)
>
> [Pale Reflections - Itch Link](https://yrgo.itch.io/pale-reflections)
<!-- > ## [*Read more about Pale Reflections*](PaleReflections#pale-reflections) -->

---
<!-- Pale Reflections - End - -->

<!-- Game jams and other things -->
## [***Other projects***](SmallProjects#smallprojects)

<img src=".\SmallProjects\img\elrich_0_tut.webp"/>

```
Game      : Small Games or experiments
Team size : solo or small teams

Side projects
    A Maze generator using path finding and an 3D offset similar to Megabonk.
    Juicing and Slicing with particles
    FPS stuff

Game Jams:
    - Local multiplayer 2D battler -
    Programmer
    - Eldrich Horror -
    Sprite animations and some programming
    - Caretaker -
    Programmer and 3D art

```



> [!NOTE]
> [*Read more about other projects*](SmallProjects#smallprojects)
> [Template-Name - Itch Link](https://yrgo.itch.io/Template-Name)
---



<!-- Game Jams and other things -->


<!-- 

## [***Template-Name***](NameOfProject#name-of-project)

<img src="Images\Template-KeyArt.jpg"/>

```
Game      : Game Name!
Role      : Gameplay Programmer,
When      : 2025 december - 2026 january
Developer : Developer Name
Engine    : Unity
Team size : solo

--short description-- 

```
> [!NOTE]
> ## [*Read more about Template-Name*](Template-Name#template-Name)
> [Template-Name - Itch Link](https://yrgo.itch.io/Template-Name)
---

 -->


